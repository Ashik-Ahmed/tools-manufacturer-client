...
const Payment = () => {
    ...
    const { data: order, orderLoading } = useQuery('tool', () => fetch(url).then(res => res.json()));
    const { data: product, productLoading } = useQuery('product', () => fetch(`http://localhost:5000/tools/${order.productId}`).then(res => res.json()));
